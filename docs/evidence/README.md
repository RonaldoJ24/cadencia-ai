...
| Claim | File | Command |
|---|---|---|
| 149 TypeScript tests pass on Node 26 | [tests-typescript.txt](tests-typescript.txt) | `npm test` |
| The same 149 pass on Node 22 | [tests-typescript-node22.txt](tests-typescript-node22.txt) | `node --test` under `node@22` |
| 113 service tests pass, each listed | [tests-python.txt](tests-python.txt) | `uv run --project service --frozen pytest service -vv` |
...
- **Model quality.** That comes from the pre-registered evaluation
  ([evals/PREREGISTRATION.md](../../evals/PREREGISTRATION.md)), committed with
  each run: the [first run](../../evals/runs/2026-09-24-freeze-v1/report.md) and
  its [blind rating](../../evals/runs/2026-09-24-freeze-v1/ratings.md), and the
  [follow-up on Luna's refusals](../../evals/runs/2026-09-25-followup-v2/report.md).
  Development checks against the real model (prompt probes, dry runs, the demo's
  recordings) are described in pull requests but are not evidence and are not
  quoted as results.
