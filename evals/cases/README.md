...
The evaluation needs 100 to 150 goals written by you, the owner. They are the
test set, so they must not come from an agent or be tuned to the prompts. This
guide explains the format and how to label each case. Coverage minimums and the
rest of the protocol are in [`../PREREGISTRATION.md`](../PREREGISTRATION.md).

## The file

Write the cases in `evals/cases/cases.jsonl`, one JSON object per line.
`TEMPLATE.jsonl` has three filled examples. Copy the shape, not the content. When
you're done, check the file:

```bash
node --experimental-strip-types evals/validate.ts evals/cases/cases.jsonl
```

The validator lists every problem, counts each coverage quota, and flags cases
that are close to texts used during development. Keep or rewrite a flagged case
as you see fit.
...
| `text` | yes | The goal as a person would type it, 1 to 2,000 characters. Write it the way you would say it, not the way the model likes it |
...
| `answer` | no | If you expect a clarifying question, the answer a person would give. The runner sends it once, after the question |
...
The rest can be anything you care about: study, music, writing, work projects,
languages, craft.
...
- Don't include personal data about real people.
